// M2 web demo frontend.
//  - The transcript streams as ONE clean growing line (locked text + volatile tail).
//  - Cleanup + formatting run once on finalize: the Final Output box shows a
//    loading indicator while it computes, then the polished text.
//  - The "what was removed" diff animates once, over the finished transcript.
const WS_URL =
...
type ServerMsg =
  ...
  | { type: "correction"; raw: string; cleanText: string; ops: Op[]; latencyMs: number; valid: boolean }
...
function handle(m: ServerMsg) {
  ...
  else if (m.type === "formatted") { finalOut.textContent = m.text; }
  else if (m.type === "error") setStatus("err", "error: " + m.message);
  else if (m.type === "done") { setStatus("done", "done — inserted"); teardownAudio(); resetButtons(); }
}
...
function connect(mode: "demo" | "live"): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    ws.onerror = () => { setStatus("err", "cannot reach dev server at " + WS_URL); reject(new Error("ws")); };
    ws.onclose = () => { ws = null; };
  });
}
...
async function startLive() {
  try {
    ...
    setStatus("err", "microphone permission denied");
    resetButtons();
    return;
  }
  reset();
...
demoBtn.onclick = async () => { reset(); finalOut.innerHTML = TYPING; buttonsBusy(); stopBtn.disabled = true; await connect("demo"); };

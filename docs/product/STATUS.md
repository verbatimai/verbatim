...
3. M3 **Phase 3.0 spikes ✅** and **Phase 3.1 ✅** done (see above). Pick up at **Phase 3.2** (configurable hotkey / push-to-talk), then **3.4** hardening (AX-write injection with paste fallback, `capture_focus()` before show, secure/password-field refusal) and **3.5** (client-side `@open-dictation/core` + keychain BYOK to drop the dev backend).
